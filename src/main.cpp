...
#if defined(NDEBUG)
#error "Dividendcash cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
	  ...
	  else if (nHeight <= 4001 && nHeight > 0) {
        nSubsidy = 25 * COIN;
    }
	  else if (nHeight <= 85001 && nHeight > 4001) {
        nSubsidy = 12 * COIN;
    }
...
{
    RenameThread("dividendcash-scriptch");
